...
public class MainActivity extends AppCompatActivity {
    private static final int BOUNDING_BOX = R.id.mapBoundingBox;
    private static final int ROUTE = R.id.mapRoute;
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        if (m_mapFragmentView.isRouteSimulationFinished) {
            if (!item.isChecked()) {
                item.setChecked(true);

                switch (item.getItemId()) {
                    case BOUNDING_BOX:
                        m_mapFragmentView.setMapType(MapType.MAP_BOUNDING_BOX);
                        return true;
                    case ROUTE:
                        m_mapFragmentView.setMapType(MapType.ROAD_RADIUS);
                        return true;
                }
            }
        } else {
            Toast.makeText(this, "You can't change map when simulation is running.",
                    Toast.LENGTH_LONG).show();
        }
        return true;
    }
