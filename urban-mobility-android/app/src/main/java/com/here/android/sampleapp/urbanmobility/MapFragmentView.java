...
public class MapFragmentView {
    ...
    private void initUMRequestButtons() {
                ...
                ResponseListener<StationSearchResult> stationSearchListener = new ResponseListener<StationSearchResult>() {
                    ...
                    public void onSuccess(StationSearchResult result) {
                        ...
                        if(stationList.size()>0) {
                            ...
                            Toast.makeText(m_activity, "No station information yet", Toast.LENGTH_SHORT).show();
                        }
                    }
                };

                /*
                 * StationSearchRequest discovers transit stations by searching around a given location,
                 * filtered by station name, maximum results, search radius, and so on
                 */
                GeoCoordinate searchCoordinate = new GeoCoordinate(49.2852595, -123.1132904);
                String stationName = null;
                StationSearchRequest stationRequest = new RequestManager().createStationSearchRequest(searchCoordinate, stationName, stationSearchListener);
        ...
        Button departureBoardRequestButton = (Button) m_activity.findViewById(R.id.departureBoardRequestBtn);
                ...
                ResponseListener<DepartureBoard> departureBoardListener = new ResponseListener<DepartureBoard>() {
                    ...
                    public void onSuccess(DepartureBoard result) {
                        ...
                        if (s_ResultList.size()>0) {
                            GeoCoordinate stationCoordinate = new GeoCoordinate(s_ResultList.get(0).getStation().getAddress().getCoordinate());
                            ...
                            m_map.setZoomLevel(16.7);
                            /* Enable the button to open the resultListView to present the departure results */
                            m_detailButton.setVisibility(View.VISIBLE);
                        } else {
                            Toast.makeText(m_activity, "No departure information yet", Toast.LENGTH_SHORT).show();
                        }
                    }
                };

                /*
                 * DepartureBoardRequest discovers departure information by searching around a given location,
                 * filtered by station Ids, date and time, maximum results, and so on
                 */
                GeoCoordinate searchCoordinate = new GeoCoordinate(49.2852595, -123.1132904);
                /* Waterfront Station Canada Line, Canada Line TO YVR */
                String stationId = "414508979";
                DepartureBoardRequest departureBoardRequest = new RequestManager().createDepartureBoardRequest(searchCoordinate, stationId, departureBoardListener);
            ...
            public void onClick(View v) {
                ...
                ResponseListener<CitySearchResult> citySearchListener = new ResponseListener<CitySearchResult>() {
                    ...
                    public void onSuccess(CitySearchResult result) {
                        ...
                        if(cityList.size()>0) {
                            GeoCoordinate cityCoordinate = new GeoCoordinate(cityList.get(0).getLocation());
                            ...
                            m_cityLinesCount.setText("City Lines: " + Integer.toString(cityList.get(0).getTransportsCount()));
                            m_cityStopsCount.setText("City Stops: " + Integer.toString(cityList.get(0).getStopsCount()));
                            m_cityCoverageQuality.setText("City Coverage Quality: " + qualityFormatter.format(cityList.get(0).getQuality()));
                        } else {
                            Toast.makeText(m_activity, "No city information yet", Toast.LENGTH_SHORT).show();
                        }
                    }
                };

                /*
                 * CitySearchRequest discovers city coverage information by searching cities with names that begin with the query string
                 * filtered by maximum results, and so on
                 */
                String cityName = "Rome";
                CitySearchRequest citySearchRequest = new RequestManager().createCitySearchRequest(cityName, citySearchListener);
        ...
        Button nearbyCoverageRequestButton = (Button) m_activity.findViewById(R.id.nearbyCoverageRequestBtn);
            ...
            public void onClick(View v) {
                ...
                ResponseListener<NearbyCoverageResult> nearbyCoverageListener = new ResponseListener<NearbyCoverageResult>() {
                    ...
                    public void onSuccess(NearbyCoverageResult result) {
                         ...
                         * e.g. Burnaby Village GeoCoordinate(49.2380074,-122.9736308) to show city of Burnaby
                        */
                        if (cityResult != null) {
                            GeoCoordinate cityCoordinate = new GeoCoordinate(cityResult.getLocation());
                            /* Add map marker to the city */
                            addMarkerAtResult(cityCoordinate);
                            /* Set map center to the city */
                            m_map.setCenter(cityCoordinate, Map.Animation.NONE);
                            ...
                            m_cityLinesCount.setText("City Lines: " + Integer.toString(cityResult.getTransportsCount()));
                            m_cityStopsCount.setText("City Stops: " + Integer.toString(cityResult.getStopsCount()));
                            m_cityCoverageQuality.setText("City Coverage Quality: " + qualityFormatter.format(cityResult.getQuality()));
                        }
                        /*
                         * If there is no transit stop within 2 kilometers of the location, the server returns
                         * the first five nearest transit stops outside of the 2 kilometers area;
                         * e.g. GeoCoordinate(50.104127, -123.0715547) to show 5 station in Whistler
                         */
                            ...
                            if (stationList.size()>0) {
                                 ...
                                 */
                                    ...
                                    /* Set map center to the first station result */
                                        m_map.setCenter(stationResult.getAddress().getCoordinate(), Map.Animation.NONE);
                                        m_map.setZoomLevel(14);
                                    }
                                }
                            } else {
                                Toast.makeText(m_activity, "No station coverage information yet in " + exploreResult.getRadius() + "m", Toast.LENGTH_SHORT).show();
                            }
                        }
                        /* No coverage information all */
                        else {
                            Toast.makeText(m_activity, "No coverage information yet in " + result.getRadius() + "m", Toast.LENGTH_SHORT).show();
                        }
                    }
                };

                /*
                 * NearbyCoverageRequest coverage information around a given location,
                 * if the city coverage information contains the given location is available, then this city coverage information is returned;
                 * else if there is no transit stop within 2km of the given location, then the first five nearest transit stops outside of this
                 * given location are returned.
                 */
                GeoCoordinate exploreCoordinate = new GeoCoordinate(50.104127, -123.0715547);
                NearbyCoverageRequest nearbyCoverageRequest = new RequestManager().createNearbyCoverageRequest(exploreCoordinate, nearbyCoverageListener);
